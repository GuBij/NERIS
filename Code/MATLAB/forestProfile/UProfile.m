function U=UProfile(z,intFGF,option)
...
%  C1=(2*uStar/(kappa*sh*gh))/(besseli(1,gh)+besseli(0,g0)*besselk(1,gh)/besselk(0,g0));
 ...
 for j=1:n
   if z(j) >= h
     U(j)=uStar/kappa*(log((z(j)-d)/z0)+2/3*log(1+1.5*zStar/(mu*(z(j)-d)))*exp(-mu*(z(j)-d)/zStar)-2/3*log(1+1.5*zStar/(mu*z0))*exp(-mu*z0/zStar));
   elseif nargin==3 & strcmp(option,'Wang')
     g=2*sqrt(A*z(j)/h);
     U(j)=C1*besseli(0,g)+C2*besselk(0,g)+Up;
   else
     U(j)=uh*exp(-eta*intFGF(j));
   end
 end
